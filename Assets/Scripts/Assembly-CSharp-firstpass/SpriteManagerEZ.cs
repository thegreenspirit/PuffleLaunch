...
public class SpriteManagerEZ : MonoBehaviour
{
	...
	protected int EnlargeArrays(int count)
	{
		...
		for (int i = 0; i < num; i++)
		{
			this.sprites[i].SetBuffers(this.vertices, this.UVs, this.UVs2, this.colors);
		}
		for (int j = num; j < this.sprites.Length; j++)
		{
			...
			this.sprites[j].SetBuffers(this.vertices, this.UVs, this.UVs2, this.colors);
	...
	{
		GameObject gameObject = (GameObject)global::UnityEngine.Object.Instantiate(prefab, position, rotation);
		SpriteRoot spriteRoot = (SpriteRoot)gameObject.GetComponent(typeof(SpriteRoot));
		this.AddSprite(gameObject);
		return spriteRoot;
	}
