...
public class CrashRotation : EZAnimation
{
	...
	{
		this.type = EZAnimation.ANIM_TYPE.CrashRotation;
		this.pingPong = false;
	}
	...
	public static CrashRotation Do(GameObject sub, Vector3 mag, Vector3 oscill, float dur, float delay, EZAnimation.CompletionDelegate startDel, EZAnimation.CompletionDelegate del)
	{
		CrashRotation crashRotation = (CrashRotation)EZAnimator.instance.GetAnimation(EZAnimation.ANIM_TYPE.CrashRotation);
	...
	public override bool Start(GameObject sub, AnimParams parms)
	{
		...
		this.Start(sub, sub.transform.localEulerAngles, parms.vec, parms.vec2, parms.duration, parms.delay, null, new EZAnimation.CompletionDelegate(parms.transition.OnAnimEnd));
		return true;
	}
	...
	protected const float PIx2 = 6.2831855f;
