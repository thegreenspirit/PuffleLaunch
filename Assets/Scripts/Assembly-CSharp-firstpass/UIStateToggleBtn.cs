...
public class UIStateToggleBtn : AutoSpriteControlBase
{
			...
			for (int i = 0; i < this.states.Length; i++)
			{
				...
				for (int j = 0; j < this.layers.Length; j++)
				{
					...
					else
					{
						...
						if (this.stateIndices[j, i] != -1)
						{
							this.transitions[i].list[0].AddSubSubject(this.layers[j].gameObject);
						}
					}
				}
				if (this.spriteText != null)
				{
					this.transitions[i].list[0].AddSubSubject(this.spriteText.gameObject);
				}
			}
			for (int j = 0; j < this.layers.Length; j++)
			{
	...
	{
		GUILayout.BeginHorizontal(new GUILayoutOption[] { GUILayout.MaxWidth(50f) });
