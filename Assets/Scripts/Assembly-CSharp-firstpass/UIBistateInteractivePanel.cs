...
public class UIBistateInteractivePanel : UIPanelBase
{
	...
	public bool IsShowing
	{
		get
		{
			return this.m_panelState == UIBistateInteractivePanel.STATE.SHOWING;
		}
	}
	...
	protected void SetPanelState(UIBistateInteractivePanel.STATE s)
	{
		...
		if (this.dismissOnPeerClick || this.dismissOnOutsideClick)
		{
			if (this.m_panelState == UIBistateInteractivePanel.STATE.SHOWING)
			{
				...
				{
					UIManager.instance.AddMouseTouchPtrListener(new UIManager.PointerInfoDelegate(this.ClickListener));
					this.lastListenerType = POINTER_INFO.POINTER_TYPE.MOUSE_TOUCHPAD;
				}
				else
				{
					UIManager.instance.AddRayPtrListener(new UIManager.PointerInfoDelegate(this.ClickListener));
					this.lastListenerType = POINTER_INFO.POINTER_TYPE.MOUSE_TOUCHPAD;
				}
			}
			else if ((this.lastListenerType & POINTER_INFO.POINTER_TYPE.MOUSE_TOUCHPAD) == this.lastListenerType)
			{
				UIManager.instance.RemoveMouseTouchPtrListener(new UIManager.PointerInfoDelegate(this.ClickListener));
			}
			else
			{
				UIManager.instance.RemoveRayPtrListener(new UIManager.PointerInfoDelegate(this.ClickListener));
			}
		}
		if (this.m_panelState == UIBistateInteractivePanel.STATE.SHOWING)
		{
