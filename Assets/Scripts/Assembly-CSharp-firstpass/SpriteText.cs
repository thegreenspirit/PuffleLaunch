...
public class SpriteText : MonoBehaviour, IUseCamera
{
	...
	protected virtual void Init()
	{
		...
		if (this.font == null && UIManager.Exists())
		{
			this.font = UIManager.instance.defaultFont;
		}
		if (this.meshRenderer.sharedMaterial == null && UIManager.Exists())
		{
			this.meshRenderer.sharedMaterial = UIManager.instance.defaultFontMaterial;
			if (this.meshRenderer.sharedMaterial != null)
			{
				this.texture = this.meshRenderer.sharedMaterial.mainTexture;
			}
		}
		else if (this.meshRenderer.sharedMaterial != null)
		{
			this.texture = this.meshRenderer.sharedMaterial.mainTexture;
		}
		if (this.texture == null && Application.isPlaying)
		{
	...
	protected void CreateMesh()
	{
		...
		if (this.persistent)
		{
			global::UnityEngine.Object.DontDestroyOnLoad(this.mesh);
		}
	}
	...
	protected void ProcessString(string str)
	{
		...
		if (this.parseColorTags)
		{
			...
			do
			{
				num4 = str.IndexOf(SpriteText.colDel[0], num4);
				if (num4 != -1)
				{
			...
			do
			{
				num4 = str.IndexOf(SpriteText.colDel[1], num4);
				if (num4 != -1)
				{
					this.colorTags.Add(num4);
					num4++;
		...
		if (this.colorTags.Count < 1)
		{
			...
			if (this.maxWidth > 0f && this.multiline)
			{
				...
				for (int j = 0; j < str.Length; j++)
				{
					...
					{
						num3 -= this.spriteFont.GetAdvance(str[j]) * this.worldUnitsPerTexel * this.characterSpacing;
					}
					if (num3 < 0f)
					{
						if (j == num)
						...
						if (num3 < 0f)
						{
							if (this.displaySB.Length > 0)
							{
								...
								num3 = this.maxWidth - this.spriteFont.GetAdvance(str[j]) * this.worldUnitsPerTexel * this.characterSpacing;
		...
		while (i < str.Length)
		{
			...
			{
				if (string.Compare(str, i, SpriteText.colDel[0], 0, SpriteText.colDel[0].Length) == 0)
				{
					i += SpriteText.colDel[0].Length;
				...
				else if (string.Compare(str, i, SpriteText.colDel[1], 0, SpriteText.colDel[1].Length) == 0)
				{
					i += SpriteText.colDel[1].Length;
					int num5 = str.IndexOf(']', this.colorTags[num4]) - i;
					if (num5 < 0)
					{
						num5 = str.Length - i;
					...
					this.colorInserts.Add(this.displaySB.Length);
					this.cols.Add(this.ParseHexColor(str.Substring(i, num5)));
					i += num5 + 1;
				}
				num4++;
			}
			else if (this.maxWidth > 0f && this.multiline)
			{
				...
				{
					num3 -= this.spriteFont.GetAdvance(str[i]) * this.worldUnitsPerTexel * this.characterSpacing;
				}
				if (num3 < 0f)
				{
					if (i == num2)
					...
					if (num3 < 0f)
					{
						...
						{
							this.displaySB.Insert(this.displaySB.Length - 1, '\n');
							num3 = this.maxWidth - this.spriteFont.GetAdvance(str[i]) * this.worldUnitsPerTexel * this.characterSpacing;
							this.newLineInserts.Add(new SpriteText.NewlineInsertInfo(this.plainSB.Length - 1, 1));
							int num6 = this.colorInserts.Count - 1;
							while (num6 >= 0 && this.colorInserts[num6] > this.newLineInserts[this.newLineInserts.Count - 1].index)
							{
								List<int> list2;
								List<int> list = (list2 = this.colorInserts);
								int num8;
								int num7 = (num8 = num6);
								num8 = list2[num8];
								list[num7] = num8 + 1;
	...
	protected void DoSingleLineTruncation()
	{
		...
		if (this.maxWidth > 0f)
		{
			...
			{
				int num3 = 0;
				float num4 = this.spriteFont.GetWidth("...") * this.worldUnitsPerTexel * this.characterSpacing;
				do
				{
					num3++;
	...
	{
		string[] array = str.Split(SpriteText.commaDelimiter);
		if (array.Length != 4)
		{
			return this.color;
		}
		return this.color * new Color(float.Parse(array[0]), float.Parse(array[1]), float.Parse(array[2]), float.Parse(array[3]));
	}
	...
	protected Color ParseHexColor(string str)
	{
		...
		try
		{
			...
			color = this.color * new Color((float)num / 255f, (float)num2 / 255f, (float)num3 / 255f, (float)num4 / 255f);
		}
		catch
		{
			color = this.color;
	...
	public void UpdateMesh()
	{
		...
		else
		{
			...
			if (this.stringContentChanged)
			{
				...
				{
					this.lines = this.displayString.Split(SpriteText.newLineDelimiter);
					flag2 = false;
				}
			}
			if (flag2 || this.lines == null || this.lines.Length == 1)
			{
	...
	protected Vector3 GetStartPos_SingleLine(float baseHeight, float width)
	{
		...
		case SpriteText.Anchor_Pos.Upper_Center:
			return new Vector3(width * -0.5f, 0f, this.offsetZ);
		case SpriteText.Anchor_Pos.Upper_Right:
			return new Vector3(-width, 0f, this.offsetZ);
		case SpriteText.Anchor_Pos.Middle_Left:
			return new Vector3(0f, baseHeight * 0.5f, this.offsetZ);
		case SpriteText.Anchor_Pos.Middle_Center:
			return new Vector3(width * -0.5f, baseHeight * 0.5f, this.offsetZ);
		case SpriteText.Anchor_Pos.Middle_Right:
			return new Vector3(-width, baseHeight * 0.5f, this.offsetZ);
		case SpriteText.Anchor_Pos.Lower_Left:
			return new Vector3(0f, baseHeight, this.offsetZ);
		case SpriteText.Anchor_Pos.Lower_Center:
			return new Vector3(width * -0.5f, baseHeight, this.offsetZ);
		case SpriteText.Anchor_Pos.Lower_Right:
			return new Vector3(-width, baseHeight, this.offsetZ);
		default:
			return new Vector3(0f, 0f, this.offsetZ);
		}
	}
	...
	protected float GetLineBaseline(int numLines, int lineNum)
	{
		...
		case SpriteText.Anchor_Pos.Middle_Center:
		case SpriteText.Anchor_Pos.Middle_Right:
			return num4 * 0.5f + (float)lineNum * -this.lineSpaceSize + num2;
		case SpriteText.Anchor_Pos.Lower_Left:
		case SpriteText.Anchor_Pos.Lower_Center:
		case SpriteText.Anchor_Pos.Lower_Right:
			return num4 + (float)lineNum * -this.lineSpaceSize + num2;
		default:
			return 0f;
		}
	}
	...
	protected void Layout_Single_Line()
	{
		...
		vector = this.GetStartPos_SingleLine(num, num2);
		this.topLeft = vector;
		this.bottomRight = new Vector3(this.topLeft.x + num2, this.topLeft.y - num);
		this.Layout_Line(vector, this.displayString, 0);
		if (this.displayString.Length < this.capacity)
		{
	...
	protected void Layout_Multiline(string[] lines)
	{
		...
		int num2 = 0;
		float num3 = this.lineSpaceSize - this.characterSize;
		float num4 = this.characterSize * (float)lines.Length + num3 * ((float)lines.Length - 1f);
		...
		case SpriteText.Anchor_Pos.Upper_Left:
			...
			this.topLeft = zero;
			this.bottomRight = new Vector3(num, -num4, this.offsetZ);
			break;
		case SpriteText.Anchor_Pos.Upper_Center:
			zero = new Vector3(num * -0.5f, 0f, this.offsetZ);
			this.topLeft = zero;
			this.bottomRight = new Vector3(num * 0.5f, -num4, this.offsetZ);
			break;
		case SpriteText.Anchor_Pos.Upper_Right:
			zero = new Vector3(-num, 0f, this.offsetZ);
			this.topLeft = zero;
			this.bottomRight = new Vector3(0f, -num4, this.offsetZ);
			break;
		case SpriteText.Anchor_Pos.Middle_Left:
			zero = new Vector3(0f, num4 * 0.5f, this.offsetZ);
			this.topLeft = zero;
			this.bottomRight = new Vector3(num, num4 * -0.5f, this.offsetZ);
			break;
		case SpriteText.Anchor_Pos.Middle_Center:
			zero = new Vector3(num * -0.5f, num4 * 0.5f, this.offsetZ);
			this.topLeft = zero;
			this.bottomRight = new Vector3(num * 0.5f, num4 * -0.5f, this.offsetZ);
			break;
		case SpriteText.Anchor_Pos.Middle_Right:
			zero = new Vector3(-num, num4 * 0.5f, this.offsetZ);
			this.topLeft = zero;
			this.bottomRight = new Vector3(0f, num4 * -0.5f, this.offsetZ);
			break;
		case SpriteText.Anchor_Pos.Lower_Left:
			zero = new Vector3(0f, num4, this.offsetZ);
			this.topLeft = zero;
			this.bottomRight = new Vector3(num, 0f, this.offsetZ);
			break;
		case SpriteText.Anchor_Pos.Lower_Center:
			zero = new Vector3(num * -0.5f, num4, this.offsetZ);
			this.topLeft = zero;
			this.bottomRight = new Vector3(num * 0.5f, 0f, this.offsetZ);
			break;
		case SpriteText.Anchor_Pos.Lower_Right:
			zero = new Vector3(-num, num4, this.offsetZ);
			this.topLeft = zero;
			this.bottomRight = new Vector3(0f, 0f, this.offsetZ);
			break;
		}
	...
	protected void BuildCharacter(int vertNum, int charNum, Vector3 upperLeft, ref SpriteChar ch)
	{
		...
		this.vertices[vertNum + 1].y = upperLeft.y - ch.UVs.height * this.worldUnitsPerUV.y;
		this.vertices[vertNum + 1].z = upperLeft.z;
		this.vertices[vertNum + 2] = this.vertices[vertNum + 1];
		Vector3[] array = this.vertices;
		int num = vertNum + 2;
		array[num].x = array[num].x + ch.UVs.width * this.worldUnitsPerUV.x;
		...
		this.UVs[vertNum + 1].y = ch.UVs.y;
		this.UVs[vertNum + 2].x = ch.UVs.xMax;
		this.UVs[vertNum + 2].y = ch.UVs.y;
		...
		if (this.clipped)
		{
			if (this.vertices[vertNum].x < this.localClipRect.x)
			{
				...
				float num2 = (this.localClipRect.x - this.vertices[vertNum].x) / (this.vertices[vertNum + 2].x - this.vertices[vertNum].x);
				this.vertices[vertNum].x = (this.vertices[vertNum + 1].x = this.localClipRect.x);
				this.UVs[vertNum].x = (this.UVs[vertNum + 1].x = Mathf.Lerp(this.UVs[vertNum].x, this.UVs[vertNum + 2].x, num2));
			}
			else if (this.vertices[vertNum + 2].x > this.localClipRect.xMax)
			{
				...
				float num3 = (this.localClipRect.xMax - this.vertices[vertNum].x) / (this.vertices[vertNum + 2].x - this.vertices[vertNum].x);
				this.vertices[vertNum + 2].x = (this.vertices[vertNum + 3].x = this.localClipRect.xMax);
				this.UVs[vertNum + 2].x = (this.UVs[vertNum + 3].x = Mathf.Lerp(this.UVs[vertNum].x, this.UVs[vertNum + 2].x, num3));
			}
			if (this.vertices[vertNum].y > this.localClipRect.yMax)
			{
				...
				float num4 = (this.vertices[vertNum].y - this.localClipRect.yMax) / (this.vertices[vertNum].y - this.vertices[vertNum + 1].y);
				this.vertices[vertNum].y = (this.vertices[vertNum + 3].y = this.localClipRect.yMax);
				this.UVs[vertNum].y = (this.UVs[vertNum + 3].y = Mathf.Lerp(this.UVs[vertNum].y, this.UVs[vertNum + 1].y, num4));
			}
			else if (this.vertices[vertNum + 2].y < this.localClipRect.y)
			{
				...
				float num5 = (this.vertices[vertNum].y - this.localClipRect.y) / (this.vertices[vertNum].y - this.vertices[vertNum + 1].y);
				this.vertices[vertNum + 1].y = (this.vertices[vertNum + 2].y = this.localClipRect.y);
				this.UVs[vertNum + 1].y = (this.UVs[vertNum + 2].y = Mathf.Lerp(this.UVs[vertNum].y, this.UVs[vertNum + 1].y, num5));
			}
		}
	}
	...
	protected void Layout_Line(Vector3 startPos, string txt, int charIdx)
	{
		...
		if (spriteChar != null)
		{
			Vector3 vector = startPos + new Vector3(spriteChar.xOffset * this.worldUnitsPerTexel, spriteChar.yOffset * this.worldUnitsPerTexel);
			this.BuildCharacter(charIdx * 4, charIdx, vector, ref spriteChar);
		}
		for (int i = 1; i < txt.Length; i++)
		{
			...
			if (spriteChar != null)
			{
				startPos.x += spriteChar.GetKerning((int)txt[i - 1]) * this.worldUnitsPerTexel * this.characterSpacing;
				Vector3 vector = startPos + new Vector3(spriteChar.xOffset * this.worldUnitsPerTexel, spriteChar.yOffset * this.worldUnitsPerTexel);
				this.BuildCharacter((charIdx + i) * 4, charIdx + i, vector, ref spriteChar);
			}
		}
	}
	...
	private void OnEnable()
	{
		...
		if (this.parentControl is AutoSpriteControlBase)
		{
			AutoSpriteControlBase autoSpriteControlBase = (AutoSpriteControlBase)this.parentControl;
	...
	public virtual void Copy(SpriteText s)
	{
		...
		this.removeUnsupportedCharacters = s.removeUnsupportedCharacters;
		this.parseColorTags = s.parseColorTags;
	...
	public string Text
	{
		...
		set
		{
			...
			{
				this.ProcessString(this.spriteFont.RemoveUnsupportedCharacters(value));
			}
			else
			{
	...
	public Vector3 GetInsertionPointPos(int charIndex)
	{
		...
		if (this.meshString.Length < 1)
		{
			float num = (float)this.spriteFont.BaseHeight * this.worldUnitsPerTexel;
			return base.transform.TransformPoint(this.GetStartPos_SingleLine(num, 0f).x, this.GetLineBaseline(1, 1), this.offsetZ);
		}
		int num2 = 1;
		if (charIndex >= this.displayString.Length)
		{
			num2 = 0;
		}
		int num3;
		int num4;
		int num5;
		int displayLineCount = this.GetDisplayLineCount(charIndex, out num3, out num4, out num5);
		charIndex = Mathf.Min(charIndex, this.displayString.Length - 1);
		if (charIndex < num4)
		{
			this.GetDisplayLineCount(charIndex - 1, out num3, out num4, out num5);
		}
		float num6 = this.spriteFont.GetWidth(this.displayString, num4, charIndex - num2) * this.worldUnitsPerTexel * this.characterSpacing;
		switch (this.anchor)
		{
		case SpriteText.Anchor_Pos.Upper_Left:
		case SpriteText.Anchor_Pos.Middle_Left:
		case SpriteText.Anchor_Pos.Lower_Left:
			...
			{
				num6 += this.totalWidth * 0.5f - this.spriteFont.GetWidth(this.displayString, num4, num5) * this.worldUnitsPerTexel * this.characterSpacing * 0.5f;
			}
			else if (this.alignment == SpriteText.Alignment_Type.Right)
			{
				num6 += this.totalWidth - this.spriteFont.GetWidth(this.displayString, num4, num5) * this.worldUnitsPerTexel * this.characterSpacing;
			}
			break;
		case SpriteText.Anchor_Pos.Upper_Center:
		case SpriteText.Anchor_Pos.Middle_Center:
		case SpriteText.Anchor_Pos.Lower_Center:
			...
			{
				num6 -= this.totalWidth * 0.5f;
			}
			else if (this.alignment != SpriteText.Alignment_Type.Right)
			{
				num6 -= this.spriteFont.GetWidth(this.displayString, num4, num5) * this.worldUnitsPerTexel * this.characterSpacing * 0.5f;
			}
			break;
		case SpriteText.Anchor_Pos.Upper_Right:
		case SpriteText.Anchor_Pos.Middle_Right:
		case SpriteText.Anchor_Pos.Lower_Right:
			...
			{
				num6 -= this.totalWidth;
			}
			else
			{
				num6 += -1f * this.spriteFont.GetWidth(this.displayString, num4, num5) * this.worldUnitsPerTexel * this.characterSpacing;
			}
			break;
		}
		return base.transform.TransformPoint(num6, this.GetLineBaseline(displayLineCount, num3), this.offsetZ);
	}
	...
	public int GetNearestInsertionPoint(Vector3 point)
	{
		...
		if (displayLineCount > 1)
		{
			...
			{
				float num4 = point.y - (this.GetLineBaseline(displayLineCount, i) + this.BaseHeight * 0.5f);
			...
			int num5 = 0;
			int num6 = 1;
			while (num5 < this.displayString.Length && num6 < num3)
			{
				if (this.displayString[num5] == '\n')
				{
					num6++;
					num = num5 + 1;
				}
		...
		int num7 = num;
		int num8 = num;
		while (num8 < this.displayString.Length && this.displayString[num8] != '\n')
		{
			if (!char.IsWhiteSpace(this.displayString[num8]))
			{
				num7 = num8 + 1;
				int num9 = num8 * 4;
				float num10 = this.vertices[num9].x + 0.5f * (this.vertices[num9 + 2].x - this.vertices[num9].x);
				if (num10 >= point.x)
				{
					num7 = num8;
					break;
	...
	public float TotalScreenWidth
	{
		get
		{
			...
			this.worldUnitsPerScreenPixel = Vector3.Distance(this.renderCamera.ScreenToWorldPoint(new Vector3(0f, 1f, distanceToPoint)), this.renderCamera.ScreenToWorldPoint(new Vector3(0f, 0f, distanceToPoint)));
			return this.totalWidth / this.worldUnitsPerScreenPixel;
		}
	}
	...
	public float GetWidth(string s)
	{
		...
		return this.spriteFont.GetWidth(s) * this.worldUnitsPerTexel * this.characterSpacing;
	}
	...
	public float GetScreenWidth(string s)
	{
		...
		float distanceToPoint = plane.GetDistanceToPoint(base.transform.position);
		this.worldUnitsPerScreenPixel = Vector3.Distance(this.renderCamera.ScreenToWorldPoint(new Vector3(0f, 1f, distanceToPoint)), this.renderCamera.ScreenToWorldPoint(new Vector3(0f, 0f, distanceToPoint)));
		return this.GetWidth(s) / this.worldUnitsPerScreenPixel;
	}
	...
	public Vector2 PixelSize
	{
		get
		{
			Vector2 vector = new Vector2(this.bottomRight.x - this.topLeft.x, this.topLeft.y - this.bottomRight.y);
			return new Vector2(vector.x * this.worldUnitsPerScreenPixel, vector.y * this.worldUnitsPerScreenPixel);
		}
	}
	...
	protected static string[] colDel = new string[] { "RGBA(", "[#", ")", "]" };

	protected static char[] newLineDelimiter = new char[] { '\n' };

	protected static char[] commaDelimiter = new char[] { ',' };
