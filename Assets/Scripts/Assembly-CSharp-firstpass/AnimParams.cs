...
public class AnimParams
{
	...
	public virtual void DrawGUI(EZAnimation.ANIM_TYPE type, GameObject go, IGUIHelper gui, bool inspector)
	{
		...
		if (this.duration < 0f && (type == EZAnimation.ANIM_TYPE.FadeMaterial || type == EZAnimation.ANIM_TYPE.FadeSprite || type == EZAnimation.ANIM_TYPE.FadeSpriteAlpha || type == EZAnimation.ANIM_TYPE.FadeAudio || type == EZAnimation.ANIM_TYPE.TuneAudio || type == EZAnimation.ANIM_TYPE.FadeText || type == EZAnimation.ANIM_TYPE.FadeTextAlpha || type == EZAnimation.ANIM_TYPE.Rotate || type == EZAnimation.ANIM_TYPE.Scale || type == EZAnimation.ANIM_TYPE.Translate || type == EZAnimation.ANIM_TYPE.TranslateScreen))
		{
		...
		case EZAnimation.ANIM_TYPE.AnimClip:
			...
			if (!inspector)
			{
				...
				GUILayout.Space(15f);
				this.floatVal = GUILayout.HorizontalSlider(this.floatVal, 0f, 1f, new GUILayoutOption[] { GUILayout.Width(200f) });
				GUILayout.EndHorizontal();
			}
			else
			{
		...
		case EZAnimation.ANIM_TYPE.TranslateScreen:
		{
			...
			else if (this.mode == EZAnimation.ANIM_MODE.FromTo)
			{
				this.vec = gui.Vector3Field("Start Pos:", this.vec);
				GUILayout.BeginHorizontal(new GUILayoutOption[0]);
				if (GUILayout.Button(new GUIContent("Use Current", "Uses the object's current value for this field"), new GUILayoutOption[0]))
				{
					this.vec = ezscreenPlacement.screenPos;
					GUI.changed = true;
				}
				if (GUILayout.Button(new GUIContent("Set as Current", "Applies the displayed values to the current object."), new GUILayoutOption[0]))
				{
					ezscreenPlacement.screenPos = this.vec;
				...
				if (GUILayout.Button(new GUIContent("Use Current", "Uses the object's current value for this field"), new GUILayoutOption[0]))
				{
					this.vec2 = ezscreenPlacement.screenPos;
					GUI.changed = true;
				}
				if (GUILayout.Button(new GUIContent("Set as Current", "Applies the displayed values to the current object."), new GUILayoutOption[0]))
				{
					ezscreenPlacement.screenPos = this.vec2;
					ezscreenPlacement.PositionOnScreen();
				}
				GUILayout.EndHorizontal();
			}
