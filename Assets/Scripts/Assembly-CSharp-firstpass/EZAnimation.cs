...
public abstract class EZAnimation : IEZLinkedListItem<EZAnimation>
{
	...
	{
		return -delta * Mathf.Cos(time / duration * 1.5707964f) + delta + start;
	}

	public static float sinusOut(float time, float start, float delta, float duration)
	{
		return delta * Mathf.Sin(time / duration * 1.5707964f) + start;
	}

	public static float sinusInOut(float time, float start, float delta, float duration)
	{
		return -delta / 2f * (Mathf.Cos(3.1415927f * time / duration) - 1f) + start;
	}
	...
	public static float spring(float time, float start, float delta, float duration)
	{
		...
		num = (Mathf.Sin(num * 3.1415927f * (0.2f + 2.5f * num * num * num)) * Mathf.Pow(1f - num, 2.2f) + num) * (1f + 1.2f * (1f - num));
	...
	public static float elasticIn(float time, float start, float delta, float duration, float amplitude, float period)
	{
		...
		{
			num = period / 6.2831855f * Mathf.Asin(delta / amplitude);
		}
		return -(amplitude * Mathf.Pow(2f, 10f * (time -= 1f)) * Mathf.Sin((time * duration - num) * 6.2831855f / period)) + start;
	}
	...
	public static float elasticOut(float time, float start, float delta, float duration, float amplitude, float period)
	{
		...
		return amplitude * Mathf.Pow(2f, -10f * time) * Mathf.Sin((time * duration - num) * 6.2831855f / period) + delta + start;
	}
	...
	public static float elasticInOut(float time, float start, float delta, float duration, float amplitude, float period)
	{
		...
		if (time < 1f)
		{
			return -0.5f * (amplitude * Mathf.Pow(2f, 10f * (time -= 1f)) * Mathf.Sin((time * duration - num) * 6.2831855f / period)) + start;
		}
		return amplitude * Mathf.Pow(2f, -10f * (time -= 1f)) * Mathf.Sin((time * duration - num) * 6.2831855f / period) * 0.5f + delta + start;
	}
	...
	public static float bounceOut(float time, float start, float delta, float duration)
	{
		...
		return delta * (7.5625f * (time -= 0.95454544f) * time + 0.984375f) + start;
	}
