...
public class FadeMaterial : EZAnimation
{
	...
	public override bool Start(GameObject sub, AnimParams parms)
	{
		...
		{
			this.Start(sub.GetComponent<Renderer>().material, parms.mode, parms.color, parms.color2, EZAnimation.GetInterpolator(parms.easing), parms.duration, parms.delay, null, new EZAnimation.CompletionDelegate(parms.transition.OnAnimEnd));
		}
		else
		{
			this.Start(sub.GetComponent<Renderer>().material, parms.mode, sub.GetComponent<Renderer>().material.color, parms.color, EZAnimation.GetInterpolator(parms.easing), parms.duration, parms.delay, null, new EZAnimation.CompletionDelegate(parms.transition.OnAnimEnd));
		}
		return true;
	}
