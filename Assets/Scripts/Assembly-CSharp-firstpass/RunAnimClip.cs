...
public class RunAnimClip : EZAnimation
{
	...
	public override bool Step(float timeDelta)
	{
		...
		if (!this.playedYet)
		{
			...
			{
				this.subject.GetComponent<Animation>().Play(this.m_clip);
			}
			else
			{
				this.subject.GetComponent<Animation>().Blend(this.m_clip, this.blending, this.duration);
			}
			this.playedYet = true;
			return true;
		}
		if (this.subject.GetComponent<Animation>().IsPlaying(this.m_clip))
		{
			return true;
		}
		this._end();
